...
select nombres_usu, nombre_per, url_men, insert_mpe, update_mpe, delete_mpe, select_mpe 
from menu men
inner join menu_perfil mpe
    on mpe.id_menu_mpe = men.id_men
inner join perfil per
    on mpe.id_perfil_mpe = per.id_per
inner join usuario_perfil upe
    on upe.id_per_upe = per.id_per
inner join usuario usu
    on upe.documento_usu_upe = usu.documento_usu
where correo_usu = 'davison_jaramillo23251@elpoli.edu.co';
